<div class="bg-gray-700 rounded-md p-4">
    <div class="uppercase tracking-widest border-b border-gray-500 mb-1 flex justify-between">
        <div>
            <button wire:click="openListAdd">Reminder</button>
        </div>
        <div class="text-sm ">
            @if ($settings->private)
                ({{$this->getTodoReminders()->where('private', true)->count()}})
            @else
                ({{$this->getTodoReminders()->where('private', false)->count()}})
            @endif
        </div>
    </div>
    ...
    <div>
        @if ($settings->private)
            @foreach ($this->getTodoReminders()->where('private', true) as $todo)
                ...
                @elseif(\Carbon\Carbon::now()->gt(\Carbon\Carbon::parse($todo->remind_date)))
                ...
                ">
                    ...
                    <div class="w-full flex justify-between -mt-0.5 pb-1">
                        ...
                        @elseif(\Carbon\Carbon::now()->gt(\Carbon\Carbon::parse($todo->remind_date)))
                            ...
                            <div class="text-xs font-bold">
                                {{abs($this->getDaysTo($todo->remind_date))-1}} days passed
                            </div>
                        @else
                            <div class="text-xs font-bold">
                                {{$todo->remind_date}}
                            ...
                            <div class="text-xs font-bold">
                                Days:{{$this->getDaysTo($todo->remind_date)+1}}
                            </div>
                        @endif
                    </div>
            ...
            @endforeach
        @else
            @foreach ($this->getTodoReminders()->where('private', false) as $todo)
                <div class="text-sm border-b border-gray-600 px-1
                @if ($todo->remind_date == date('Y-m-d'))
                    bg-blue-600 hover:bg-blue-500 border border-white rounded-md mt-1
                @elseif(\Carbon\Carbon::now()->gt(\Carbon\Carbon::parse($todo->remind_date)))
