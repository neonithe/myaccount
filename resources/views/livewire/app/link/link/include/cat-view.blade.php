...
<div>
    <div class="flex justify-end gap-2">
        <div class="w-full">
            ...
            <div><input wire:model="catName" type="text" class="bg-gray-700 border border-gray-500 rounded-md py-1 w-full"></div>
        </div>

        <div class="mt-6">
            <button wire:click="addCat" class="border rounded-md bg-gray-700 py-1 px-1">Save</button>
        </div>
        ...
        <div class="mt-4">
            ...
            <div>
                <div class="flex gap-2 flex-wrap">
                    <div class="grow">
                        <input wire:model.live="searchCat" type="text" class="bg-gray-700 border border-gray-500 rounded-md py-1 w-full" placeholder="Search">
                    </div>
                </div>
            </div>
            ...
            @foreach ($listCat as $item)
                <div class="flex justify-between mt-4 gap-2 border-b border-gray-700 pb-2">
                    <div class="grow">
                        <input wire:keydown.enter="changeCat({{$item->id}}, 'name', $event.target.value, 'Name')" value="{{$item->name}}" type="text" class="bg-gray-800 border border-gray-800 rounded-md py-1 w-full">
                    </div>
                    <div class="flex justify-end gap-1">
                        <button wire:click="deleteCat({{$item->id}})" class="border border-gray-300 hover:bg-red-500 py-1.5 px-1.5 rounded-md"><x-app.icons.trash class="h-5 w-5"/></button>
                    </div>
                </div>
            @endforeach
